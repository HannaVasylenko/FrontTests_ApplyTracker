...
def test_search_field_by_note_name_desktop(desktop_user_authorization_for_note_page, config_data: dict):
    board = desktop_user_authorization_for_note_page.notes
    search_term = config_data['search_note_name_value']
    zero_results_locator = "//form/..//p[text()='0 search results']"
    board.type_in_search_field_on_notes_page(search_term)
    if desktop_user_authorization_for_note_page.locator(zero_results_locator).is_visible():
        print("Verification: '0 search results' is visible.")
    else:
        notes_names_values = board.collect_notes_search_result_values(config_data['search_note_name_value'])
...
def test_note_search_field_with_nonexistent_value_desktop(desktop_user_authorization_for_note_page, config_data: dict):
    board = desktop_user_authorization_for_note_page.notes
    ...
    board.type_in_search_field_on_notes_page(nonexistent_search_term)
    desktop_user_authorization_for_note_page.locator(search_results_locator).wait_for(state='visible')
    expect(desktop_user_authorization_for_note_page.locator(empty_result_locator)).to_have_text(f"{empty_search_text}{nonexistent_search_term}")


def test_sort_by_old_first_desktop(desktop_user_authorization_for_note_page, config_data: dict):
    notes_page = desktop_user_authorization_for_note_page.notes
    ...
    notes_page.click_on_sort_by_btn_on_notes_page()
...
def test_sort_alphabetically_desktop(desktop_user_authorization_for_note_page, config_data: dict):
    notes_page = desktop_user_authorization_for_note_page.notes
...
def test_add_note_desktop(desktop_user_authorization_for_add_note_test_window, config_data: dict):
    notes = desktop_user_authorization_for_add_note_test_window.notes
    ...
    expect(desktop_user_authorization_for_add_note_test_window.locator(toast_locator)).to_have_text(success_add_toast)
    desktop_user_authorization_for_add_note_test_window.locator(toast_locator).wait_for(state='hidden')
    notes.delete_note(note_name)
    expect(desktop_user_authorization_for_add_note_test_window.locator(toast_locator)).to_have_text(success_delete_toast)
